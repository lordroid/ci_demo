...
typedef int (*fn_I2cReadMem)(void * context, unsigned char slave_addr, uint32_t sub_addr, uint32_t rd_len, unsigned char*rd_buf);
typedef int (*fn_I2cWriteMem)(void * context, unsigned char slave_addr, uint32_t sub_addr, uint32_t wr_len, unsigned char*wr_buf);
typedef int (*fn_SetResetPin) ( void* pCallbackContext,int bSet);


/*
 * The SetupI2cWriteMemCallback sets the I2cWriteMem callback function.
 * PARAMETERS
 *    pCallbackContext [in] - A pointer to a caller-defined structure of data items to be passed as 
 *							  the context parameter of the callback routine each time it is called. 
 *    I2cWritePtr      [in] - A pointer to a i2cwirte callback routine, which is to write I2C data. 
 *							  The callback routine must conform to the following prototype:
 * 
 *                        int (*fn_I2cWriteMem)(void * context, unsigned char slave_addr, unsigned long sub_addr, 
 *								unsigned long write_len, unsigned char* write_buf,);
 *
 *                        The callback routine parameters are as follows:
 *                        context          [in] - A pointer to a caller-supplied context area as specified 
 *												  in the CallbackContext parameter of SetupI2cWriteMemCallback. 
 ...
 *
 *    cbMaxWriteBufSize  [in] - Specify the maximux transfer size for a I2c continue writing with 'STOP'. 
 *								This is limited in I2C bus Master device. The size can not less then 4.
 * RETURN
 *    None
 */
void SetupI2cWriteMemCallback(void *pCallbackContext, fn_I2cWriteMem I2cWritePtr, uint32_t cbMaxWriteBufSize);
 ...
 * 
 * PARAMETERS
 *    pCallbackContext    [in] - A pointer to a caller-defined structure of data items to be passed as the 
 *								 context parameter of the callback routine each time it is called. 
 *
 *    pI2cReadMemPtr      [in] - A pointer to a i2cwirte callback routine, which is to write I2C data. 
 *								 The callback routine must conform to the following prototype:
 *
 *                        int (*fn_I2cReadMem)(void * context,unsigned char slave_addr,unsigned long sub_addr,unsigned long rd_len, void* rd_buf);
 *                        The callback routine parameters are as follows:
 *                         context            [in]  - A pointer to a caller-supplied context area as specified in 
 ...
 *
 *   cbMaxI2CRead         [in] - Specify the maximux transfer size for a I2c continue read. The size can not less then 4.
 * RETURN
 *      None
 * 
 */
void SetupI2cReadMemCallback(void *pCallbackContext, fn_I2cReadMem I2cWriteThenReadPtr, uint32_t cbMaxI2CRead);
 ...
 *								 the context parameter of the callback routine each time it is called. 
 *
 *    SetResetPinPtr      [in] - A pointer to a i2cwirte callback routine, which is to write I2C data. 
 *								 The callback routine must conform to the following prototype:
 *
 *                        int (*fn_SetResetPin)(void * pCallbackContext,int bSet );
 *                        The callback routine parameters are as follows:
 *                         pCallbackContext [in] - A pointer to a caller-supplied context area as specified in 
 *												   the CallbackContext parameter of SetupI2cWriteMemCallback. 
 *                         bSet             [in] - Indicates whether to high or low the GPIO pin.
 * RETURN
 *    If the operation completes successfully, the return value is ERRNO_NOERR. Otherwise, return ERRON_* error code. 
 */
void SetupSetResetPin(void *pCallbackContext, fn_SetResetPin SetResetPinPtr);
