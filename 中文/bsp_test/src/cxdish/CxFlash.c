...
#define DBG_ERROR  
#define DBG_INFO   
#define LOG(_msg_)										    mprintf _msg_
...
#define LOG(_msg_)											mprintf _msg_ 
#define InitShowProgress(MaxPos)							mprintf( "Downloading ... %3d%%", 0);
#define ShowProgress(curPos, bForceRedraw, eState,MaxPos)	mprintf( "\b\b\b\b%3d%%", (char) (((curPos) *100)/MaxPos) );
#endif
...
unsigned int g_cbMaxI2cWrite = 32;
unsigned int g_cbMaxI2cRead = 16;
void *g_pContextI2cWrite = NULL;
void *g_pContextI2cWriteThenRead = NULL;
void *g_pContextSetResetpin  = NULL;
unsigned char g_bChipAddress  = 0x41;                   /*Specify the i2c chip address*/
 ...
 *    None
 */
extern void SetupI2cWriteMemCallback(void * pCallbackContext,fn_I2cWriteMem I2cWritePtr,unsigned int cbMaxWriteBufSize)
{
 ...
 *      None
 */
extern void SetupI2cReadMemCallback( void * pCallbackContext,fn_I2cReadMem pI2cReadMemPtr,unsigned int cbMaxI2CRead)
{
 ...
 */
extern void SetupSetResetPin(void * pCallbackContext,fn_SetResetPin  SetResetPinPtr)
{
...
const char*   get_loader_err_msg(int id)
{
	unsigned int i;
...
int i2c_start_automode(const byte *base, dword part_size, dword *pRet)
{
	...
	hdrs =  (unsigned int*) malloc(2*SFS_BLOCKSIZE/sizeof(uint));
	if(NULL == hdrs){
		mprintf("no memory!\n");
		return -1;
	}
...
const char*  get_spi_mem_name(uint id)
{
	unsigned int i;
	...
	for(;;){
		...
		switch(state)
		{
			...
			case TRANSFER_STATE_BAD_PACKET://# device->host: bad packet. retry
				...
				break;
			//case TRANSFER_STATE_PACKET_READY:
...
int download_boot_loader(const unsigned char *  loader_data, unsigned int num_byte)
{
	...
	if(g_SetResetPinPtr){
		g_SetResetPinPtr(g_pContextSetResetpin,0xFF); //set Device RESET.
	}
		...
		int legacy_device)
{
	...
	do{
		...
		if( NULL == g_I2cReadMemPtr||	NULL == g_I2cWriteMemPtr ){
			err_no = -ERRNO_I2CFUN_NOT_SET;
			LOG((DBG_ERROR "i2C function is not set.\n"));
			break;
		}
		...
		if (g_bBootTimeUpgrade){
			...
			LOG((DBG_INFO "the g_bBootTimeUpgrade is 0!\n"));
			g_bIsRenoDev = 1; 
		...
		LOG((DBG_INFO "Max burst write     	: %d bytes\n", g_cbMaxI2cWrite));
		LOG((DBG_INFO "Max burst read      	: %d bytes\n", g_cbMaxI2cRead));
		LOG((DBG_INFO "Endianness          	: %s-Endian \n", little_endian? "Little":"Big"));
		LOG((DBG_INFO "SFS version			: %d.%d.%d.%d \n", (uint8_t) (g_sfs_version>>24),(uint8_t) (g_sfs_version>>16),(uint8_t) (g_sfs_version>>8), (uint8_t) (g_sfs_version))); 
		if( ((uint32_t) -1) != g_firmware_version[0])
...
int SendCmdV(Command *cmd)
{
	...
	i2c_sub_burst_write(4, (size-1)*4, (unsigned char*)&i2c_data[1]);
    
    // write word 0 to address 0
    //i2c_write (0x0, &i2c_data[0], 1);
	i2c_sub_burst_write(0, 4, (unsigned char*)&i2c_data[0]);
    sys_mdelay(200);
    ...
    while (elapsed_ms < REPLY_POLL_TIMEOUT_MSEC)
    {
        ...
        i2c_sub_burst_read(0, 4, (unsigned char *)&i2c_data[0]);
        if(cmd->reply==1)
            break;
        sys_mdelay(REPLY_POLL_INTERVAL_MSEC);
        elapsed_ms += REPLY_POLL_INTERVAL_MSEC;
    ...
    if( cmd->reply == 1 )
    {
        if (cmd->num_32b_words > MAX_COMMAND_SIZE)
            goto error;
        if (cmd->num_32b_words > 0)
			i2c_sub_burst_read(8, cmd->num_32b_words *4, (unsigned char *)&i2c_data[2]);
